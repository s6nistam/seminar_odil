...
def plot_2d(
    ...
    fig.savefig(path, dpi=dpi, bbox_inches="tight", pad_inches=0.01, transparent=transparent)
    plt.close(fig)
...
def get_cmap(name):
    ...
    nodes = data[:, 0]
    colors = data[:, 1:]
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list(name, list(zip(nodes, colors)))
    return cmap
